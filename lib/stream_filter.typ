...
(* type of filter on stream:
   R means keep the element
   K means keep it,
   And there must be infinitely many K's *)
type RK(Y,X) = [ R of Y | K of X ]
type SF(α,β) = να X.μβ Y.{} → RK(Y,X)
type F = SF(∞,∞)

(* A variant of the same type that helps the termination checker *)
type UF = μ Y.{} → RK(Y,F)
check F ⊆ UF
check UF ⊆ F
...
(* but work if we unroll the fixpoint twice *)
...
(* composition of filters, need unrolling twice with UF *)
...
(* and three times with F, but Too slow
...
(* a variant, less lazy: only the K constructor is lazy, globally identical *)
...
(* A third variant moving unit inside, again similar,
   but compose fails to type check, see below for
   explanation *)
...
(*The following example fails, because in the
  typing of R (compose3 ...), the term is not normal
...
(* unfolding if not using UF, but with size preservation *)
...
(* idem using the less lazy type for filter *)
