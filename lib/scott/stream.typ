...
(* FIXME: fail to find the proper state*)
?val cons : ∀A A → Stream(A) → Stream(A) = ΛA fun a s → triple (pair a s) pi1 pi2
...
val test  : ∀A∀P (Stream0(P,A) → Stream(A)) =
  ΛAΛP (fun (x:Stream0(P,A)) → (x:GStream(A) with S = Pair(P,T(A,P))))

val coiter : ∀S∀A (S → Pair(S,A)) → S → Stream(A) = ΛSΛA fun f n →
  (triple (pair n
...
val head'' : ∀P∀A G(A,P) → U(A,P) = ΛPΛA fun f c → head' (f (fun x → x):(P → P) (pi1 c))
val tail'' : ∀P∀A G(A,P) → T(A,P) = ΛPΛA fun f c →
  triple (pair (tail' (f (fun x → x):(P → P) (pi1 c))) (pi2 c)) (head'' f) (pi2 c)

val corec : ∀P∀A G(A,P) → P → Stream(A) = ΛPΛA fun f n →
