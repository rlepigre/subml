...
let type_constr name size =
  let aux i = Printf.sprintf "C%i of H(%c)" (i+1) uppercase.(i) in
...
let _ =
  ...
  Printf.printf "type H(X) = ∀Y (X → Y) → Y\n%!";
  ...
  List.iter (fun p1 -> List.iter (fun p2 ->
    let aux = function
      ...
      | (i, false) -> Printf.printf "%c" uppercase.(i)
    in
    let no = if test_permutations p1 p2 then "" else "not " in
    Printf.printf "check %sT" no;
    List.iter aux p1;
    Printf.printf " ⊂ T";
    List.iter aux p2;
    Printf.printf "\n%!"
